...
public class Main {
    ...
    public static void solve(){
        ...
        //index, releaseTime
        PriorityQueue<int[]> blocked = new PriorityQueue<int[]>((a,b)->a[1]-b[1]);
        HashSet<Integer> blockedIndices = new HashSet();
        
        for(int i = 0; i < q; i++){
            int type = in.nextInt();
            while(blocked.size() > 0 && blocked.peek()[1] <= i){
                    int[] curr = blocked.poll();
                    blockedIndices.remove(curr[0]);
            }
            switch(type){
                case 1: 
                    int b = in.nextInt();
                    blockedIndices.add(b-1);
                    blocked.add(new int[]{b-1, i+t});
                    break;
                case 2: 
                    int s1 = in.nextInt()-1;
                    int p1 = in.nextInt()-1;
                    int s2 = in.nextInt()-1;
                    int p2 = in.nextInt()-1;
                    char temp = strs[s1][p1];
                    strs[s1][p1] = strs[s2][p2];
                    strs[s2][p2] = temp;
                    break;
                case 3: 
                    if(checkEqual(strs[0], strs[1], blockedIndices)){
