...
public class Main {
    ...
    public static void solve(){
        long noOfPeople = in.nextInt();
        long goldCoins = in.nextInt();
        long silverPerGld = in.nextInt();
        
        long val = (long)Math.ceil((double)silverPerGld/2)-1;

        if(goldCoins*silverPerGld - val*noOfPeople <= 0){
            printWriter.println((long)goldCoins*silverPerGld);
        }else{
            long rem = (goldCoins*silverPerGld)-((noOfPeople-1)*(val));
            long g = rem/silverPerGld;
            long r = rem%silverPerGld;
            if(r >= val+1){
                g++;
            }
            printWriter.println((goldCoins-g)*silverPerGld);
        }
